...
public class FisherWave : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        else
        {
            ...
            delta = Vector3.MoveTowards(this.transform.position, initial + target, step);
            delta.x = initial.x;
            this.transform.position = delta;

            if (Mathf.Abs(this.transform.position.y) < yDeltaReached)
                reached = true;
        }
    }
